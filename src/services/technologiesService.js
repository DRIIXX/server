...
class TechnologiesService {
  ...
  getList() {
    return technologies.map(
      t => new TechnologyDTO(t.id, nameToUpperPipe(t.name), t.description, t.category, t.efficiency)
    );
  }
  ...
  searchByName(nameQuery) {
    ...
    const filtered = technologies.filter(t => t.name.toLowerCase().includes(lowerQuery));
    return filtered.map(t => new TechnologyDTO(t.id, nameToUpperPipe(t.name), t.description, t.category, t.efficiency));
  }
  ...
  getById(id) {
    ...
    if (tech) {
      return new TechnologyDTO(tech.id, nameToUpperPipe(tech.name), tech.description, tech.category, tech.efficiency);
    }
    return null;
  }
  ...
  create(newTechData) {
    ...
    return new TechnologyDTO(newTech.id, nameToUpperPipe(newTech.name), newTech.description, newTech.category, newTech.efficiency);
  }
  ...
  update(id, updatedData) {
    ...
    if (updatedTech) {
      return new TechnologyDTO(updatedTech.id, nameToUpperPipe(updatedTech.name), updatedTech.description, updatedTech.category, updatedTech.efficiency);
    }
    return null;
  }
